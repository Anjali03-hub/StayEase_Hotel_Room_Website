...
//     View booking
    useEffect(()=>
    {
        axios.get("http://localhost:8000/viewbooking")
...
<div class="content">
<h2 class="fw-bold h-font text-center" >View Booking</h2>
            ...
            <div className="w-70 vh-40 bg-white justify-content-center rounded p-4 ">
              ...
              <table class="table  border-1 table-hover ">
                ...
                {
                    users.map((user)=>{
                        return <tr><td>{user._id}</td><td>{user.fullname}</td><td>{user.gmailid}</td><td>{user.mobileno}</td><td>{user.checkoutdate}</td>
                       <td>{user.checkindate}</td><td>{user.child}</td><td>{user.adult}</td> <td>{user.aadhar}</td><td>
